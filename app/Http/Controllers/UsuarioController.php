...
class UsuarioController  extends Controller
{
    ...
    public function registrar(Request $_request)
    {
        ...
        try {
            ...
            $token = $user->createToken('tasks api')->accessToken;

            // Responder con el token y un mensaje de éxito
            return response()->json(['success' => 'Usuario creado con éxito.', 'token' => $token], 201);
    ...
    public function create(Request $_request)
    {
        ...
        $_request->validate([
            'usuario_rut' => 'required|unique:users,rut|max:12',
        ...
        $datos = $_request->only('usuario_rut', 'usuario_nombre', 'usuario_apellido', 'usuario_email', 'usuario_password', 'usuario_rePassword', 'usuario_rol_id');
    ...
    public function update(Request $_request, $_id)
    {
        ...
        $_request->validate([
            'usuario_rut' => 'required|string|max:12|unique:users,rut,' . $_id,
        ...
        $datos = $_request->only('_token', 'usuario_rut', 'usuario_nombre', 'usuario_apellido', 'usuario_email', 'usuario_password', 'usuario_rePassword', 'usuario_imagen', 'usuario_rol_id');
        
        $cambios = 0;

        // solo si es distinto actualiza
        if ($registro->rut != $datos['usuario_rut']) {
            $registro->rut = $datos['usuario_rut'];
            $cambios++;
        }
        ...
        if ($registro->apellido != $datos['usuario_apellido']) {
            $registro->apellido = $datos['usuario_apellido'];
            $cambios++;
        }
